...
mod tests {
    ...
    fn parses_platform_handshake_rejection() {
        ...
        let error = HyperStackError::from_http_response(response);
        assert!(matches!(
            error,
            HyperStackError::HandshakeRejected {
                status: 403,
    ...
    fn parses_token_endpoint_error_response() {
        let error = HyperStackError::from_auth_response(
            429,
        ...
        assert!(matches!(
            error,
            HyperStackError::AuthRequestFailed {
                status: 429,
    ...
    fn parses_rate_limit_close_reason() {
        let error = HyperStackError::from_close_reason(
    ...
    fn parses_unknown_close_reason_without_code() {
        let error = HyperStackError::from_close_reason("server maintenance")
    ...
    fn socket_issue_error_uses_issue_retryability() {
        let error = HyperStackError::from_socket_issue(SocketIssue {

...
impl Config {
    ...
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        Ok(Self {
            ...
            port: env::var("PORT")
                ...
                .parse()
                .unwrap_or(8080),
            issuer: env::var("ISSUER").unwrap_or_else(|_| "hyperstack-auth".to_string()),
            default_audience: env::var("DEFAULT_AUDIENCE")
                .unwrap_or_else(|_| "hyperstack".to_string()),
            default_ttl_seconds: env::var("DEFAULT_TTL_SECONDS")
                .unwrap_or_else(|_| "300".to_string())
                .parse()
                .unwrap_or(300),
            signing_key_path: env::var("SIGNING_KEY_PATH")
                .unwrap_or_else(|_| "/etc/hyperstack/auth/signing.key".to_string()),
            verifying_key_path: env::var("VERIFYING_KEY_PATH")
                .unwrap_or_else(|_| "/etc/hyperstack/auth/verifying.key".to_string()),
    ...
    pub fn generate_keys_if_missing(
        &self,
    ) -> anyhow::Result<(hyperstack_auth::SigningKey, hyperstack_auth::VerifyingKey)> {
        use hyperstack_auth::keys::KeyLoader;
        use std::path::Path;

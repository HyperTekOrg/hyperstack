...
mod tests {
    ...
    impl Env for TestEnv {
        ...
        fn credentials_file_path_display(&self) -> String {
            "<test:~/.hyperstack/credentials.toml>".to_string()
        }
    }
    ...
    fn env_var_wins_over_file() {
        let env = TestEnv::default()
            .with_var(ENV_VAR_API_KEY, "hsk_from_env")
            .with_credentials("api_key = \"hsk_from_file\"");
        let r = resolve_with(&env, None, "wss://foo.stack.usehyperstack.com").unwrap();
    ...
    fn parses_new_url_keyed_table() {
        let content = "[keys]\n\
                       \"https://api.usehyperstack.com\" = \"hsk_new\"\n";
        assert_eq!(
            parse_credentials_content(content, "https://api.usehyperstack.com").as_deref(),
            Some("hsk_new")
        );
    }
    ...
    fn new_format_respects_api_url_selector() {
        let content = "[keys]\n\
                       \"https://api.usehyperstack.com\" = \"hsk_prod\"\n\
    ...
    fn hosted_url_without_any_key_is_error() {
        let env = TestEnv::default();
        let err = resolve_with(&env, None, "wss://any.stack.usehyperstack.com").unwrap_err();
        let msg = err.to_string();
        assert!(msg.contains("no HyperStack api key"), "{msg}");
        assert!(msg.contains("hs auth login"), "{msg}");
        assert!(msg.contains("HYPERSTACK_API_KEY"), "{msg}");
    ...
    fn hosted_url_detection() {
        assert!(is_hosted_websocket_url("wss://foo.stack.usehyperstack.com"));
        assert!(is_hosted_websocket_url(
            "wss://a-b-c.stack.usehyperstack.com"
        ));
        assert!(is_hosted_websocket_url(
            "wss://a.stack.usehyperstack.com/socket"
        ));
        assert!(is_hosted_websocket_url(
            "wss://a.stack.usehyperstack.com:443"
        ));

//! Module processing for arete streams.
//!
//! This module handles processing of `#[arete]` attributes applied to modules,
...
/// Process a module annotated with `#[arete(...)]`.
///
/// This handles:
...
pub fn parse_proto_files_from_attr(attr: TokenStream) -> syn::Result<ParsedProtoAttrs> {
    let arete_attr = parse::parse_stream_spec_attribute(attr)?;

    parse_proto_files_from_parsed_attr(arete_attr)
}

fn parse_proto_files_from_parsed_attr(
    arete_attr: parse::StreamSpecAttribute,
) -> syn::Result<ParsedProtoAttrs> {
    let idl_files = arete_attr.idl_files.clone();

    if arete_attr.proto_files.is_empty() {
        return Ok((Vec::new(), arete_attr.skip_decoders, idl_files));
    }
    ...
    for proto_path in &arete_attr.proto_files {
    ...
    Ok((analyses, arete_attr.skip_decoders, idl_files))
}

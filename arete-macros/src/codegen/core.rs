...
pub fn generate_set_field_code(target_field: &str, source: &MappingSource) -> TokenStream {
    match source {
        MappingSource::FromSource {
            ...
            quote! {
                if let Some(value) = ctx.data::<arete::runtime::serde_json::Value>(#field_str) {
        ...
        MappingSource::Constant(value) => {
            ...
            quote! {
                ctx.set(#target_field, arete::runtime::serde_json::from_str::<arete::runtime::serde_json::Value>(#value_str).unwrap_or(arete::runtime::serde_json::Value::Null));
            }
        }
        MappingSource::FromContext { field } => match field.as_str() {
            "slot" => {
                quote! { ctx.set(#target_field, arete::runtime::serde_json::json!(ctx.slot().unwrap_or(0))); }
            }
            "signature" => {
                quote! { ctx.set(#target_field, arete::runtime::serde_json::json!(ctx.signature().unwrap_or_default())); }
            }
            "timestamp" => {
        ...
        ParsedCondition::Comparison { field, op, value } => {
            ...
            match op {
                ComparisonOp::Equal | ComparisonOp::NotEqual => {
                    ...
                    quote! {
                        ctx.data::<arete::runtime::serde_json::Value>(#field_str)
                            .map(|v| v #op_code arete::runtime::serde_json::from_str::<arete::runtime::serde_json::Value>(#value_str).unwrap_or(arete::runtime::serde_json::Value::Null))
                            .unwrap_or(false)
                    }
                }
                    ...
                    quote! {
                        {
                            ...
                            let compare_val: Option<i64> = arete::runtime::serde_json::from_str(#value_str).ok();

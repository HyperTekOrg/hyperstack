...
pub async fn record_slot_hash(slot: u64, slot_hash: String) {
    let mut cache: tokio::sync::RwLockWriteGuard<'_, BTreeMap<u64, String>> = SLOT_HASH_CACHE.write().await;
...
pub async fn get_slot_hash(slot: u64) -> Option<String> {
    let cache: tokio::sync::RwLockReadGuard<'_, BTreeMap<u64, String>> = SLOT_HASH_CACHE.read().await;

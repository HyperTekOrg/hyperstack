...
fn run(cli: Cli) -> anyhow::Result<()> {
    match cli.command {
        Commands::Init => {
            commands::config::init(&cli.config)
        }
        Commands::Up { spec_name, branch, preview } => {
            commands::up::up(&cli.config, spec_name.as_deref(), branch, preview)
        }
        Commands::Status => {
            commands::status::status(cli.json)
        }
        Commands::Push { spec_name } => {
        ...
        Commands::Logs { build_id } => {
            let id = match build_id {
                ...
                None => {
                    ...
                    let local_specs = config::resolve_specs_to_push(config.as_ref(), None).unwrap_or_default();
                    ...
                    let builds = if local_specs.is_empty() {
                        ...
                        for ast in &local_specs {
                            if let Ok(Some(spec)) = client.get_spec_by_name(&ast.spec_name) {
                                if let Ok(builds) = client.list_builds_filtered(Some(1), None, Some(spec.id)) {
        ...
        Commands::Sdk(sdk_cmd) => match sdk_cmd {
            SdkCommands::Create(create_cmd) => match create_cmd {
                CreateCommands::Typescript { spec_name, output, package_name } => {
                    commands::sdk::create_typescript(&cli.config, &spec_name, output, package_name)
                }
            }
            SdkCommands::List => {
                commands::sdk::list(&cli.config)
            }
        },
        Commands::Config(config_cmd) => match config_cmd {
            ...
            ConfigCommands::Validate => {
                commands::config::validate(&cli.config)
            }
        },
        ...
        Commands::Spec(spec_cmd) => match spec_cmd {
            ...
            SpecCommands::Pull => {
                commands::spec::pull(&cli.config)
            }
            SpecCommands::List => {
                commands::spec::list_remote(cli.json)
            }
            SpecCommands::Versions { spec_name, limit } => {
                commands::spec::versions(&spec_name, limit, cli.json)
            }
            SpecCommands::Show { spec_name, version } => {
                commands::spec::show(&spec_name, version)
            }
            SpecCommands::Delete { spec_name, force } => {
                commands::spec::delete(&spec_name, force)
            }
        },
        Commands::Build(build_cmd) => match build_cmd {
            BuildCommands::Create { spec_name, version, ast_file, no_wait } => {
                commands::build::create(&cli.config, &spec_name, version, ast_file.as_deref(), !no_wait)
            }
            BuildCommands::List { limit, status } => {
                commands::build::list(limit, status.as_deref(), cli.json)
            }
            BuildCommands::Status { build_id, watch, json } => {
                commands::build::status(build_id, watch, json || cli.json)
            }
            BuildCommands::Logs { build_id } => {
                commands::build::logs(build_id)
            }
        },
        Commands::Deploy(deploy_cmd) => match deploy_cmd {
            DeployCommands::Info { build_id } => {
                commands::deploy::info(build_id, cli.json)
            }
            DeployCommands::List { limit } => {
                commands::deploy::list(limit, cli.json)
            }
            DeployCommands::Stop { deployment_id, force } => {
                commands::deploy::stop(deployment_id, force)
            }
            DeployCommands::Rollback { spec_name, to, build, branch, rebuild, no_wait } => {

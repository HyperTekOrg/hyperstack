...
pub fn info(build_id: i32, json: bool) -> Result<()> {
    ...
    if !json {
        println!("{} Fetching deployment info for build #{}...", "->".blue().bold(), build_id);
    }
    ...
    if build.status != BuildStatus::Completed {
        println!();
        println!("{} Build #{} is not deployed yet.", "!".yellow().bold(), build_id);
        ...
        if !build.status.is_terminal() {
            ...
            println!("  {}", format!("hyperstack build status {} --watch", build_id).cyan());
        }
...
pub fn rollback(
    ...
    println!("{} Starting rollback for '{}'...", "->".blue().bold(), spec_name);
    ...
    let target_version_id = if let Some(bid) = build_id {
        ...
        build.spec_version_id.ok_or_else(|| anyhow::anyhow!(
        ...
        let ver = versions.iter().find(|v| v.version_number == version)
            .ok_or_else(|| anyhow::anyhow!(
        ...
        println!("  Found version {} (hash: {})", ver.version_number, &ver.content_hash[..12]);
        ver.id
    } else {
        // Find previous successful deployment
        println!("{} Finding previous successful deployment...", "->".blue().bold());
        ...
        let mut successful_builds: Vec<&Build> = builds.iter()
        ...
        println!("  Current deployment: build #{} (version_id: {:?})", 
            current.id, current.spec_version_id);
        println!("  Rolling back to: build #{} (version_id: {:?})", 
    ...
    println!("{} Rollback build created (ID: {})", "✓".green().bold(), response.build_id);
    ...
    println!("  {}", format!("hyperstack build status {} --watch", response.build_id).cyan());
...
fn watch_build(client: &ApiClient, build_id: i32) -> Result<()> {
    println!("{} Watching rollback build #{}...\n", "->".blue().bold(), build_id);

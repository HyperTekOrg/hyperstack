...
pub fn create(
    ...
    if let Some(ast_path) = ast_file {
        ...
        println!("{} Build created (ID: {})", "✓".green().bold(), response.build_id);
        ...
        println!();
        println!("Track progress with: {}", format!("hyperstack build status {} --watch", response.build_id).cyan());
    ...
    let (spec_id, spec_version_id) = match (&remote_spec, version) {
        (Some(spec), Some(v)) => {
            ...
            println!("{} Found spec (id={}), looking up version {}...", 
                "✓".green().bold(), spec.id, v);
        ...
        (Some(spec), None) => {
            // Latest version
            println!("{} Found spec (id={}), using latest version...", 
                "✓".green().bold(), spec.id);
            
            let spec_with_version = client.get_spec_with_latest_version(spec.id)?;
            
            let version_id = spec_with_version.latest_version
                .as_ref()
                .map(|v| v.id)
                .ok_or_else(|| anyhow::anyhow!(
            ...
            if let Some(ver) = &spec_with_version.latest_version {
                println!("  Using version {} (hash: {})", ver.version_number, &ver.content_hash[..12]);
            }
        ...
        (None, _) => {
            ...
            println!("{} Spec not found remotely, searching for local AST...", "!".yellow().bold());
            
            // Try to find AST by spec name
            if let Some(ast) = find_ast_file(spec_name, None)? {
                println!("{} Found local AST: {}", "✓".green().bold(), ast.path.display());
...
pub fn list(limit: i64, status_filter: Option<&str>, json: bool) -> Result<()> {
    ...
    if filtered_builds.is_empty() {
        if status_filter.is_some() {
            println!("{}", format!("No builds with status '{}' found.", status_filter.unwrap_or("")).yellow());
        } else {
            println!("{}", "No builds found.".yellow());
            println!("Create a build with: {}", "hyperstack build create <spec-name>".cyan());
        }
        return Ok(());
    }
...
pub fn logs(build_id: i32) -> Result<()> {
    ...
    println!();
    println!("  {} Logs are available in the AWS Console.", "!".yellow().bold());

...
pub fn pull(config_path: &str) -> Result<()> {
    ...
    if remote_specs.is_empty() {
        ...
        println!(
            "  Run {} to upload your local specs.",
    ...
    let mut config =
        HyperstackConfig::load_optional(config_path)?.unwrap_or_else(|| HyperstackConfig {
...
pub fn versions(spec_name: &str, limit: i64, json: bool) -> Result<()> {
    ...
    if versions.is_empty() {
        ...
        println!(
            "Push a version with: {}",
            format!("hyperstack spec push {}", spec_name).cyan()
        );
        return Ok(());
    }

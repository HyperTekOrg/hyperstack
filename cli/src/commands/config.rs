...
pub fn init(config_path: &str) -> Result<()> {
    ...
    if discovered.is_empty() {
        ...
        for ast in &discovered {
            println!("    {} {} ({})", "•".dimmed(), ast.entity_name, ast.path.display());
        }
        println!();
    }
    ...
    // Build config
    let specs: Vec<SpecConfig> = discovered.iter().map(|ast| {
        SpecConfig {
    ...
    if config.specs.is_empty() {
        ...
        println!("{}", "Next steps:".bold());
        println!("  {} to verify your configuration", "hyperstack config validate".cyan());
        println!("  {} to push your specs to remote", "hyperstack spec push".cyan());
...
pub fn validate(config_path: &str) -> Result<()> {
    ...
    let config = HyperstackConfig::load(config_path)
        .context("Failed to load configuration. Run `hyperstack init` to create a configuration file.")?;
    ...
    if config.specs.is_empty() {
        ...
        println!("  Add specs to hyperstack.toml or run {} to auto-detect", "hyperstack init".cyan());
    } else {

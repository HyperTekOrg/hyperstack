...
pub fn up(config_path: &str, spec_name: Option<&str>, branch: Option<String>, preview: bool) -> Result<()> {
    ...
    if specs.len() > 1 && spec_name.is_none() {
        println!("{} Found {} specs. Deploying all...\n", "→".blue().bold(), specs.len());
    }
...
fn deploy_single_spec(client: &ApiClient, ast: &crate::config::DiscoveredAst, branch: Option<&str>) -> Result<()> {
    println!("{}", "━".repeat(50).dimmed());
    if let Some(branch_name) = branch {
        println!("{} Deploying {} (branch: {})", "→".blue().bold(), ast.spec_name.bold(), branch_name.cyan());
    } else {
    ...
    if version_response.version_is_new {
        println!("{} v{} ({})", "✓".green(), version_response.version.version_number, hash_short);
    } else {
        println!("{} v{} (up to date)", "=".blue(), version_response.version.version_number);
    }

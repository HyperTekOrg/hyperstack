...
When you point to an IDL (e.g., `ore.json`), Hyperstack reads the program name from the IDL metadata and generates a typed SDK module named `{name}_sdk` — so `ore.json` produces `ore_sdk::accounts::*` and `ore_sdk::instructions::*`. All `#[map]` and `#[aggregate]` paths use this prefix.
:::
...
:::tip[Checkpoint]
Verify that the file `.hyperstack/OreStream.stack.json` exists in your project root. This JSON file contains the compiled specification for your stack, including all entities and their field mappings.
:::

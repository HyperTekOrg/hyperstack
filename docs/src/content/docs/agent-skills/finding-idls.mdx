...
sidebar:
  order: 5
description: How to find Anchor IDL files for Solana programs when building custom Hyperstack stacks.
---

Hyperstack uses Anchor IDL files to generate type-safe bindings for your data streams. An IDL (Interface Definition Language) is a JSON file that describes a Solana program's accounts, instructions, and custom types. 
...
### 5. Manual Creation
If a program is not built with Anchor or the IDL is unavailable, you can create one manually by examining the program's source code. Tools like Kinobi or Codama can help generate IDLs for non-Anchor programs by parsing the Rust source.

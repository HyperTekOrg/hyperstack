...
:::note[This page covers the React SDK]
For the pure TypeScript SDK (`hyperstack-typescript`), see the [TypeScript SDK Overview](/sdks/typescript/overview#typescript-core-sdk-hyperstack-typescript).
:::
...
// When using take: 1 or useOne(), the return type narrows:
interface SingleItemHookResult<T> {
  data: T | undefined; // Single entity (not array)
  isLoading: boolean;
  error?: Error;
  refresh: () => void;
}
...
interface ListParams {
  ...
  limit?: number; // Max results (client-side)
  take?: number; // Server-side limit (use 1 for single item with type narrowing)
  skip?: number; // Server-side offset
...
// Single item with type narrowing (data: T | undefined)
const { data: latest } = stack.views.tokens.list.use({ take: 1 });

// Using useOne() for cleaner single-item queries
const { data: latest } = stack.views.tokens.list.useOne();

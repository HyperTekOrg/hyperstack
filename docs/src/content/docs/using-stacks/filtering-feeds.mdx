...
use hyperstack_stacks::token::{TokenStack, Token};

let hs = HyperStack::<TokenStack>::connect().await?;

// Get first 10 entities
let mut stream = hs.views.token.list()
    .listen()
    .take(10);
...
// Default views
let mut list_stream = hs.views.ore_round.list().listen();
let round_address = "some-round-address";
let mut state_stream = hs.views.ore_round.state().listen(round_address);

// Custom view - sorted by round_id desc
let mut latest_stream = hs.views.ore_round.latest().listen();
...
let mut stream = hs.views.ore_round.latest()
    .listen()
    .filter(|round| {

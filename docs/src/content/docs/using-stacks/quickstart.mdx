...
:::tip[Not using React?]
For Vue, Svelte, Node.js, or vanilla JavaScript, use the [TypeScript Core SDK](/sdks/typescript/overview#typescript-core-sdk-hyperstack-typescript) instead.
:::
...
- An existing Hyperstack stack deployed (or use a public demo)
...
Replace `your-stack` with your deployed stack name, or use `ws://localhost:8080` for local development.

---

## Step 3: Define Your Stack

Create a stack definition that matches your deployed stack's entities:

```tsx
// src/stack.ts
import { defineStack, createListView, createStateView } from "hyperstack-react";

// Define types matching your stack entities
interface Token {
  mint: string;
  name?: string;
  symbol?: string;
...
**View path format:** `{EntityName}/{mode}` where mode is `list` or `state`.
    ...
    <div>
      ...
      {tokens?.map((token) => (
        <div key={token.mint}>
          ...
          {token.price && (
            <span> - {stack.helpers.formatPrice(token.price)}</span>
          )}
        </div>
      ))}
    </div>
...
1. **HyperstackProvider** establishes a WebSocket connection to your deployed stack
...
- [Building Stacks](/building-stacks/overview) - Create custom data streams
- [Stack API Reference](/concepts/stack-api) - Full API documentation
- [CLI Commands](/cli/commands) - Deploy and manage stacks
...
| Issue                         | Solution                                                       |
| ----------------------------- | -------------------------------------------------------------- |
| "WebSocket connection failed" | Check your `websocketUrl` is correct and the stack is deployed |
| Data not updating             | Verify the view path matches your stack entity name            |

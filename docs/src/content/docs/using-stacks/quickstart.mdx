...
The TypeScript template uses `hyperstack-typescript` with the core streaming API:

```typescript
// main.ts
import { HyperStack, createUpdateStream } from "hyperstack-typescript";
...
const stream = createUpdateStream<OreRound>(
  hs.store,

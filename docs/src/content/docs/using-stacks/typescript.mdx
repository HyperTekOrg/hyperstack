...
```typescript
const hs = await HyperStack.connect(
  "wss://your-stack.stack.usehyperstack.com",
  {
  ...
  for await (const update of hs.subscribe<Token>("Token/list")) {
    if (update.type === "upsert") {
      ...
      if (token.totalVolume && token.totalVolume > 1000000) {
        console.log(`🚨 High volume token: ${token.name}`);
...
- [React Guide](/using-stacks/react) — If you're using React
- [Build Your Own Stack](/building-stacks/workflow) — Create custom data streams
- [API Reference](/sdks/typescript/overview) — Complete SDK documentation

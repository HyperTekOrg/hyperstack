    ...
    <div>
      {rounds?.map((round) => (
        <div key={round.round_id}>
          {stack.helpers.shortRoundId(round.round_id)} —{" "}
          {stack.helpers.formatMotherlode(round.motherlode)}
        </div>
      ))}
    </div>
  );
    ...
    <ul>
      {rounds?.map((round) => (
        <li key={round.round_id}>
          Round #{round.round_id} — Motherlode:{" "}
          {stack.helpers.formatMotherlode(round.motherlode)}
        </li>
      ))}
    </ul>
...
const { data } = stack.views.tokens.list.use({
  take: 10,                         // Server-side: limit entities returned
  skip: 0,                          // Server-side: pagination offset
  where: { price: { gte: 100 } },   // Client-side: filter after receiving
  limit: 10,                        // Client-side: max results to keep
});
```

For server-side filtering beyond pagination, use [custom views](/using-stacks/filtering-feeds/#server-side-filtering) defined in your stack.

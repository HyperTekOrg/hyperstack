...
The SDK uses `AsyncIterable` for streaming — modern, memory-efficient, and easy to use:
...
for await (const update of hs.views.tokens.list.watchRich()) {
  if (update.type === "updated") {
    ...
    if (update.before.price !== update.after.price) {
      console.log(
        `Price changed: ${update.before.price} → ${update.after.price}`,
      );
    }
  }
}
...
Not streaming — just fetch the current state:
  ...
  for await (const update of hs.subscribe<OreRound>("OreRound/list")) {
    if (update.type === "upsert") {
      ...
      if (round.motherlode && round.motherlode > 1000000000) {
        console.log(`🚨 High motherlode round: #${round.round_id}`);
        console.log(
          `   Motherlode: ${(round.motherlode / 1e9).toFixed(2)} SOL`,
        );
  ...
  try {
    for await (const update of hs.views.rounds.list.watch<OreRound>()) {
      ...
      switch (update.type) {
        case "upsert":
          console.log(
            `[${updateCount}] Round #${update.data.round_id} - Motherlode: ${update.data.motherlode}`,
          );
          break;
        case "patch":
          console.log(
            `[${updateCount}] Round #${update.data.round_id} updated`,
          );
          break;
        case "delete":
...
- `url` — WebSocket URL of the deployed stack

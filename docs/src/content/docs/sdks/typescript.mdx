...
// Connect with a stack definition for typed views
const hs = await HyperStack.connect("wss://ore.stack.usehyperstack.xyz", {
  stack: ORE_STACK,
});
...
Always connect with a stack definition to get typed views:
...
- `options.stack` — Stack definition for typed views
- `options.maxEntriesPerView` — Max entries per view before LRU eviction

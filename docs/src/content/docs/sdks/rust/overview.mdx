...
The SDK includes a `prelude` module that re-exports `StreamExt` from `futures-util`, so you typically don't need to add it as a direct dependency.
...
The `prelude` module re-exports all commonly needed types including `StreamExt`, so you don't need separate imports.

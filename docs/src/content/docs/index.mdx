...
<CardGrid>
  <Card title="Agent Skills" icon="star">
    **One command setup** — Install skills and your agent knows all the SDK patterns and CLI commands.

    Works with Cursor, Claude Code, Windsurf, VS Code Copilot, and 30+ other agents.

    [Install skills →](/agent-skills/overview/)

  </Card>
  <Card title="Schema Discovery" icon="magnifier">
    **Live type info** — Agents use `hs explore` to get exact entity names, field paths, and types from deployed stacks.

    Always accurate, always up to date.

    [Learn more →](/agent-skills/explore/)

...
✔ Build completed
🚀 Deployed to: wss://counter-stack.stack.usehyperstack.com
```
...
async function main() {
  ...
  const hs = await HyperStack.connect('wss://counter-stack.stack.usehyperstack.com');
  ...
  for await (const update of hs.views.counters.list.watch()) {
    if (update.type === 'upsert') {
      ...
      console.log(`  Last updated: ${new Date(counter.last_updated * 1000).toLocaleString()}`);
      console.log('');
    }
  }
}
...
- Learn more about [Stack Macros](/stacks/macros) to build complex pipelines.
- Explore [View Strategies](/stacks/strategies) for different data access patterns.

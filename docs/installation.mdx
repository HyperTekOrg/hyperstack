...
async fn main() -> anyhow::Result<()> {
    let hs = HyperStack::connect("wss://your-stack.hyperstack.xyz").await?;
    
    // List all entities
    let tokens = hs.list::<TokenEntity>().await;
    
    // Watch for real-time updates (lazy - no await needed)
    let mut stream = hs.watch::<TokenEntity>()
...
See the [Rust SDK README](https://github.com/HyperTekOrg/hyperstack-oss/tree/main/rust/hyperstack-sdk) for full documentation.
...
- [Creating Stacks](/guides/defining-stacks) - Create custom data streams

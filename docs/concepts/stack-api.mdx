...
The Stack API is the client-side interface to your deployed Arete stream. It provides type-safe access to views, transactions, and helpers.
...
```typescript
import { defineStack, useArete, createListView, createStateView } from 'arete-react';
...
```typescript
function useArete<TStack>(stack: TStack): StackClient<TStack>
```
...
interface StackClient<TStack> {
  ...
  store: AreteStore;    // Direct store access
  runtime: AreteRuntime; // Runtime instance
}
```
...
function TokenDashboard() {
  const stack = useArete(TokenStack);
...
Monitor WebSocket connection status via `useArete`:
...
function ConnectionIndicator() {
  const { connectionState, isConnected } = useArete(MY_STACK);

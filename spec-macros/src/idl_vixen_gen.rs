...
/// Generates the vixen runtime setup function (with registries - for backward compatibility)
pub fn generate_vixen_runtime(
    idl: &IdlSpec,
    program_id: &str,
    resolver_hooks: &[ResolverHookSpec],
) -> TokenStream {
    let registries = generate_resolver_registries(resolver_hooks);
    let runtime = generate_vixen_runtime_without_registries(idl, program_id);
...
/// Generates the vixen runtime setup function without resolver registries
pub fn generate_vixen_runtime_without_registries(idl: &IdlSpec, _program_id: &str) -> TokenStream {
    ...
    quote! {
        ...
        /// Creates and runs a yellowstone-vixen runtime with the generated parsers
        ///
        /// # Environment Variables
        /// - `YELLOWSTONE_ENDPOINT` (required): The gRPC endpoint URL
        /// - `YELLOWSTONE_X_TOKEN` (optional): Authentication token
        /// - `RUST_LOG` (optional): Log level (default: info)
        pub async fn run_vixen_runtime() -> anyhow::Result<()> {
            ...
            if !env_loaded {
                // No .env file found. Make sure environment variables are set.
            }
            ...
            impl yellowstone_vixen::Handler<parsers::#state_enum_name, yellowstone_vixen_core::AccountUpdate> for VmHandler {
                    ...
                    -> yellowstone_vixen::HandlerResult<()>
                {
                    ...
                    // Extract slot and signature for context
                    let slot = raw_update.slot;
                    let account = raw_update.account.as_ref();
                    ...
                    let account_address = bs58::encode(
                        &raw_update.account.as_ref().unwrap().pubkey
                    ).into_string();

                    // Add account address to event value for potential use in resolvers
                    if let Some(obj) = event_value.as_object_mut() {
                        obj.insert("__account_address".to_string(), serde_json::json!(account_address));
                    }

                    // println!("🔍 Account update: type={}, address={}", event_type, account_address);
                    ...
                    match resolver_result {
                        ...
                        hyperstack_interpreter::resolvers::KeyResolution::QueueUntil(discriminators) => {
                    ...
                    let context = hyperstack_interpreter::UpdateContext::new(slot, signature);

                    match vm.process_event_with_context(&self.bytecode, event_value, event_type, Some(&context)) {
                        Ok(mutations) => {
                            // Computed fields are now evaluated by the VM via the registered evaluator
                            Ok(())
                        }
                        Err(e) => {
            ...
            impl yellowstone_vixen::Handler<parsers::#instruction_enum_name, yellowstone_vixen_core::instruction::InstructionUpdate> for VmHandler {
                    ...
                    -> yellowstone_vixen::HandlerResult<()>
                {
                    ...
                    // println!("   Event Value with Named Accounts:");
                    // println!("{}", serde_json::to_string_pretty(&event_value).unwrap_or_default());

                    let bytecode = self.bytecode.clone();
                    let mut vm = self.vm.lock().unwrap();

                    // Debug: Log event structure for IxState types
                    if std::env::var("HYPERSTACK_DEBUG_EVENTS").is_ok() && event_type.ends_with("IxState") {
                        tracing::debug!("📋 Processing {}", event_type);
                        tracing::debug!("   raw_update.accounts.len() = {}", static_keys_vec.len());
                        if let Some(accounts) = event_value.get("accounts") {
                            tracing::debug!("   event_value.accounts keys: {:?}", accounts.as_object().map(|o| o.keys().collect::<Vec<_>>()));
                        } else {
                            tracing::warn!("   ⚠️ event_value has NO 'accounts' key!");
                        }
                    }
                    ...
                    // Process instruction through VM
                    match vm.process_event_with_context(&bytecode, event_value.clone(), event_type, Some(&context)) {
                        Ok(mut mutations) => {
                            // Computed fields are now evaluated by the VM via the registered evaluator
                            ...
                            if !hooks.is_empty() {
                                        ...
                                        obj.iter()
                                            .filter_map(|(k, v)| {
                                                if let Some(s) = v.as_str() {
                                                    ...
                                                    // Debug: Account value is not a string
                                                    None
                                                }
                                            })
                                ...
                                // Extract instruction data (the "data.data" field from event_value for instruction args)
                                // The structure is: event_value = { accounts: {...}, data: { amount, ... } }
                                let instruction_data = event_value.get("data")
                                    .and_then(|d| d.get("data"))
                                    .or_else(|| event_value.get("data"))
                                ...
                                // vm.registers_mut() and vm.path_cache() access different fields than the
                                // ReverseLookupUpdater trait methods (which access pda_reverse_lookups).
                                // This is safe because we know these fields don't overlap.
                                ...
                                // We use unsafe to split the borrow - passing raw pointer-derived references
                                let mut ctx = hyperstack_interpreter::resolvers::InstructionContext::with_metrics(
                                    accounts,
                                    0, // state_id
                                    &mut *vm,
                                    unsafe { (*vm_ptr).registers_mut() },
                                    0, // state_reg - typically register 0 holds the state
                                ...
                                if !dirty_fields.is_empty() {
                                    ...
                                    if let Ok(patch) = vm.extract_partial_state(0, &dirty_fields) {
                                        // Find or create mutation for the current entity
                                        // The primary key should be in the state - extract it from event_value
                                        if let Some(mint) = event_value.get("accounts").and_then(|a| a.get("mint")).and_then(|m| m.as_str()) {
                                            // Find existing mutation or create new one
                                            if let Some(existing_mutation) = mutations.iter_mut().find(|m| {
                                                m.key.as_str() == Some(mint)
                                            }) {
                                                // Deep merge patch into existing mutation
                                                if let (Some(existing_patch_obj), Some(new_patch_obj)) =
                                                    (existing_mutation.patch.as_object_mut(), patch.as_object()) {
                                                ...
                                                // Computed fields were already evaluated by the VM
                                            } else {
                                                // Create new mutation
                                                mutations.push(hyperstack_interpreter::Mutation {
                                                    export: "PumpfunToken".to_string(), // TODO: Get from entity name
                                                    key: serde_json::Value::String(mint.to_string()),
                                                    patch,
                                                });
                                            }
                                        }
                                        ...
                                        match vm.process_event_with_context(&bytecode, account_data, &update.account_type, Some(&update_context)) {
                                            Ok(pending_mutations) => {
                                                if !pending_mutations.is_empty() {
                                                    // Note: In standalone mode without mutations channel, these are lost
                                                    // Users should use hyperstack-server integration for full functionality
                                                } else {
                                                }
                                            }
                                            Err(e) => {
            ...
            yellowstone_vixen::Runtime::<YellowstoneGrpcSource>::builder()
                .account(Pipeline::new(account_parser, [handler.clone()]))
                .instruction(Pipeline::new(instruction_parser, [handler]))
                .build(vixen_config)
                .run_async()
                .await;
...
/// Generates the spec function for hyperstack-server integration (with registries - for backward compatibility)
...
pub fn generate_spec_function_without_registries(idl: &IdlSpec, _program_id: &str) -> TokenStream {
    ...
    quote! {
        ...
        async fn run_vixen_runtime_with_channel(
            ...
            let runtime_builder = yellowstone_vixen::Runtime::<YellowstoneGrpcSource>::builder()
                .account(account_pipeline);


            let runtime_builder = runtime_builder.instruction(instruction_pipeline);

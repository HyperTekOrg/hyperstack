...
fn generate_account_types(accounts: &[IdlAccount], types: &[IdlTypeDef]) -> TokenStream {
    let account_structs = accounts.iter().map(|account| {
        generate_account_type(account, types)
    });
...
fn generate_instruction_types(instructions: &[IdlInstruction], _types: &[IdlTypeDef]) -> TokenStream {
    let instruction_structs = instructions.iter().map(|instruction| {
        generate_instruction_type(instruction)
    });
...
fn generate_custom_types(types: &[IdlTypeDef]) -> TokenStream {
    let type_defs = types.iter().map(|type_def| {
        generate_custom_type(type_def)
    });

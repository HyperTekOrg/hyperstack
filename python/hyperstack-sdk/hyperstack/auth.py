"""Authentication support for HyperStack Python SDK."""
...
from hyperstack.errors import HyperStackError, AuthError
...
HOSTED_WEBSOCKET_SUFFIX = ".stack.usehyperstack.com"
...
@dataclass
class AuthConfig:
    """Configuration for HyperStack authentication.
...
def is_hosted_hyperstack_websocket_url(websocket_url: str) -> bool:
    """Check if URL is a hosted Hyperstack Cloud URL."""
    try:
...
class AuthState:
    ...
    def has_refreshable_auth(self) -> bool:
        ...
        return (
            ...
            or (
                ...
                and is_hosted_hyperstack_websocket_url(self.websocket_url)
            )
        )
    ...
    def _get_token_endpoint(self) -> Optional[str]:
        ...
        # For hosted Hyperstack URLs, use default endpoint if publishable key provided
        if self.config.publishable_key and is_hosted_hyperstack_websocket_url(
            self.websocket_url
        ):
    ...
    async def resolve_token(self, force_refresh: bool = False) -> Optional[str]:
        ...
        if is_hosted_hyperstack_websocket_url(self.websocket_url):
            raise AuthError(
                "Hosted Hyperstack websocket connections require authentication",
                AuthErrorCode.AUTH_REQUIRED,
            )
